...
/// Total mpvr chunk-body capacity for H2A (28-byte header + 0x7C00 slot).
pub const h2a_variant_storage_length: usize = 0x7C00;

#[derive(BlfChunk, PartialEq, Debug, Clone, Serialize, Deserialize, Default)]
#[Header("mpvr", 137.1)]
pub struct s_blf_chunk_halo2a_game_variant
{
...
impl BinRead for s_blf_chunk_halo2a_game_variant {
    type Args<'a> = ();
    ...
    fn read_options<R: Read + Seek>(reader: &mut R, _endian: Endian, _args: Self::Args<'_>) -> BinResult<Self> {
        ...
        let available_slot = data.len().saturating_sub(payload_byte_start)
            .min(h2a_variant_storage_length);
        ...
        let tail_end = payload_byte_start + h2a_variant_storage_length;
...
impl BinWrite for s_blf_chunk_halo2a_game_variant {
    type Args<'a> = ();
    ...
    fn write_options<W: Write + Seek>(&self, writer: &mut W, _endian: Endian, _args: Self::Args<'_>) -> BinResult<()> {
        ...
        let unknown04 = self.raw_unknown04.unwrap_or(0);
        ...
        let pad_size = h2a_variant_storage_length.saturating_sub(self.raw_gametype_data.len());
        if pad_size > 0 {
...
impl BlfChunkHooks for s_blf_chunk_halo2a_game_variant {}

impl s_blf_chunk_halo2a_game_variant {
    pub fn create(game_variant: c_game_variant) -> s_blf_chunk_halo2a_game_variant {
        s_blf_chunk_halo2a_game_variant {
            game_variant,
            ..Default::default()
        }
    }
